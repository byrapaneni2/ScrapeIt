...
class MagicBricks(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for city in cities:
            ...
            if not os.path.exists('parsed/rent/multistorey-apartment/'+city):
                os.makedirs('parsed/rent/multistorey-apartment/'+city)

            for file in os.listdir('html/rent_html/multistorey-apartment/'+city):
                file_id = file
                url = 'file://%s/html/rent_html/multistorey-apartment/%s/%s' % (root,city,file)
    ...
    def parse(self, response):
        ...
        #Area
        super_builtup_area = response.xpath("//div[@itemtype='http://schema.org/Apartment']/meta[@itemprop='floorSize']/@content").extract()
        super_builtup_area = re.findall(r'\b\d+\b', super_builtup_area[0])
        super_builtup_area = int(super_builtup_area[0])
        if super_builtup_area:
            prop_details['super_builtup_area'] = super_builtup_area
        ...
        file = 'parsed/rent/multistorey-apartment/%s/%s' % (response.meta['city'],response.meta['file_id'])
