...
class MagicBricks(scrapy.Spider):
	...
	name = "extract_urls_rent"

	def start_requests(self):
		cities = load_obj('cities')
		for city in cities:
			url = "http://www.magicbricks.com/property-for-rent/ALL-RESIDENTIAL-real-estate-%s/Page-1" % city
			yield scrapy.Request(url=url, callback=self.parse, meta={'city': city, 'page_num':1, 'urls':[]})
	def parse(self, response):
		page_num = response.meta['page_num']
		city = response.meta['city']
		urls = response.xpath("//div[@class='proHeading']//a[@class='property-sticky-link']/@href").extract()
		urls = list(set(urls))
		...
		if page_num == 1:
			...
			for page_num in range(2,max_page_num+1):
				req_url= "http://www.magicbricks.com/property-for-rent/ALL-RESIDENTIAL-real-estate-%s/Page-%d" % (city,page_num)
