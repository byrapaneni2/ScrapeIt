...
class MagicBricks(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for city in cities:
            ...
            if not os.path.exists('parsed/rent/studio-apartment/'+city):
                os.makedirs('parsed/rent/studio-apartment/'+city)

            for file in os.listdir('html/rent_html/studio-apartment/'+city):
                file_id = file
                url = 'file://%s/html/rent_html/studio-apartment/%s/%s' % (root,city,file)
    ...
    def parse(self, response):
        ...
        if balcony:
            prop_details['balconies'] = int(balcony[0])
        if floor_num:
        ...
        if units_available:
            prop_details['units_available'] = units_available[0]
        if description:
        ...
        file = 'parsed/rent/studio-apartment/%s/%s' % (response.meta['city'],response.meta['file_id'])
